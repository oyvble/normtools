...
void EM_estLoop(double *xptr, double *lambda, double *mu, double *sigmaSq, double *p1, int *n, double *epsilon) {
  ...
  //copy variables
  vec xvec(xptr, *n, false); //init vector
  vec xvecsq = xvec%xvec;  //Calc elementwise/Schur product and keeping long vector
  vec z; 
  long double sumX = sum(xvec);
  ...
  while (diff) { 
    ...
    z = -0.5*xvecsq/ *sigmaSq + xvec*( *mu / *sigmaSq + *lambda) - con; //calulcate logged expression
    z = *p1/(*p1+(1-*p1)*exp(z));
