...
#X2 = scaling(X) 
#X3=t(apply(X,1,scale))
# min(abs(X2-X3))

#Transposed version:
#X2 = scaling(t(X),transposed=T) #sample given per-row


#Also includes ROBUST Rescaling of data using MEDIAN and MAD
scaling = function(X,mad2sd=1.4826,transposed=FALSE,addAttr=TRUE, robust=FALSE,center=TRUE,scale=TRUE) {
 ...
 #Post transformation:
 if(center) X = X-CENTER #center
 if(scale) X = X/SCALE #scale

 if(addAttr) {
  if(center) attr(X, "scaled:center") <- CENTER
  if(scale) attr(X, "scaled:scale") <- SCALE
 }
