...
preprocessENmixR  <- function(rgSet, bgParaEst="oob", dyeCorr="RELIC",QCinfo=NULL, exQCsample=TRUE,exQCcpg=TRUE, exSample=NULL, exCpG=NULL, nCores=2) {
  ...
  enmix <- function(meth,bg,bgParaEst,nCores) {
    ...
    meth.o <- foreach(i=1:ncol(meth),.combine=cbind,.export=c("EM_estimateR","new_cm","enmix_adjR")) %dopar% {
      i=i;enmix_adjR(meth[,i],bg[i,],bgParaEst)}
